...
data_path = current_directory / 'data/finalq.json'
...
class Quoter:
    ...
    def create_quote(self):
        ...
        self.quote = self.chat_model.predict(quote_prompt)
        self.quote = re.sub('#(\w+)|"', '', self.quote)
        logging.info(f'{quote_prompt}\n----------')

    def inspire_image(self):
        image_prompt = IMAGE_PROMPT.format(quote=self.quote)
        dalle_prompt = self.chat_model.predict(image_prompt)

        logging.info(f'{image_prompt}\n----------')
